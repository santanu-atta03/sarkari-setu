...
function AdmitCardsContent() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      ...
      try {
        ...
        console.error(err);
        setError('Failed to load admit cards. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const updateURL = (newFilters: any) => {
    ...
    Object.entries(newFilters).forEach(([key, value]) => {
      if (value && value !== '1' && key !== 'limit' && key !== 'status' && key !== 'hasAdmitCard') {
         if (typeof value === 'string' && value.includes('All')) return;
         params.set(key, String(value));
      }
    });
    router.push(`/admit-cards?${params.toString()}`);
  };
        ...
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <h1 className="text-4xl md:text-5xl font-black text-zinc-900 dark:text-white tracking-tighter uppercase mb-2">
              Admit <span className="text-blue-600">Cards</span>
            </h1>
            <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs flex items-center gap-2">
              ...
              Download hall tickets for {totalJobs} upcoming exams
            </p>
          </div>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          <aside className="hidden lg:block w-72 shrink-0 space-y-10">
            <div className="sticky top-32 space-y-10">
               <div>
                 <div className="flex items-center justify-between mb-6">
                    <h3 className="text-xs font-black text-zinc-400 dark:text-zinc-500 uppercase tracking-[0.2em] flex items-center gap-2">
                       <MapPin size={14} className="text-blue-600" /> Filter by State
                    </h3>
               ...
               <div className="p-8 bg-blue-600/10 border border-blue-600/20 rounded-[32px] overflow-hidden relative group">
                  <div className="relative z-10 text-center">
                     <FileDown className="text-blue-600 mx-auto mb-4" size={32} />
                     <h4 className="text-lg font-black uppercase tracking-tighter leading-tight mb-2 dark:text-white">Need Help?</h4>
                     <p className="text-[10px] uppercase font-bold text-zinc-500 dark:text-zinc-400 tracking-widest mb-6 leading-relaxed">Check our guide on how to download admit cards safely.</p>
                     <button className="w-full py-3 bg-blue-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-lg shadow-blue-600/20">Read Guide</button>
                  </div>
               </div>
            </div>
          </aside>
            ...
            <AnimatePresence mode='wait'>
              {loading ? (
                ...
                >
                   ...
                   <h3 className="text-2xl font-black text-zinc-900 dark:text-white uppercase tracking-tighter">No Admit Cards</h3>
                   <p className="text-zinc-500 font-medium max-w-sm mt-2">No admit card links are available for the selected criteria at this moment.</p>
