...
export default function JobDetailsContainer({ job }: JobDetailsContainerProps) {
      ...
      content: (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="p-6 bg-zinc-50 dark:bg-white/[0.03] border border-black/5 dark:border-white/5 rounded-3xl">
            ...
            <div className="p-6 bg-zinc-50 dark:bg-white/[0.03] border border-black/5 dark:border-white/5 rounded-3xl">
               ...
               <p className="text-xl font-black text-zinc-900 dark:text-white">{job.qualification}</p>
            </div>
          </div>
      ...
      content: (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
           {[
             ...
             <div key={fee.label} className="p-6 bg-zinc-50 dark:bg-white/[0.03] border border-black/5 dark:border-white/5 rounded-3xl text-center">
                <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-2">{fee.label}</p>
                <p className="text-2xl font-black text-zinc-900 dark:text-white">₹{fee.value}</p>
             </div>
           ))}
        </div>
      )
           ...
           {job.vacancy?.breakdown?.length > 0 && (
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               {job.vacancy.breakdown.map((item: any, i: number) => (
                 <div key={i} className="flex items-center justify-between p-5 bg-zinc-50 dark:bg-white/5 border border-black/5 dark:border-white/5 rounded-2xl">
                    <span className="text-zinc-500 dark:text-zinc-400 font-bold">{item.post}</span>
                    <span className="text-zinc-900 dark:text-white font-black">{item.count}</span>
                 </div>
               ))}
             </div>
           )}
    ...
    <div className="min-h-screen bg-white dark:bg-[#0a0a0b] flex flex-col font-sans transition-colors duration-500">
      ...
      <main className="flex-1">
             ...
             {job.featuredImage ? (
               <>
                 <Image 
                  ...
                  priority
                 />
                 <div className="absolute inset-0 bg-gradient-to-t from-white dark:from-[#0a0a0b] via-white/60 dark:via-[#0a0a0b]/60 to-transparent" />
               </>
             ) : (
              ...
              <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10">
                 ...
                 >
                    <div className="flex items-center gap-4 mb-6">
                       <button 
                         onClick={() => router.back()}
                         className="w-12 h-12 bg-zinc-900/10 dark:bg-white/10 backdrop-blur-md rounded-2xl flex items-center justify-center text-zinc-900 dark:text-white hover:bg-zinc-900/20 dark:hover:bg-white/20 transition-all shadow-2xl"
                       >
                         <ArrowLeft size={24} />
                       </button>
                       ...
                       {job.isTrending && (
                          <div className="px-4 py-2 bg-orange-500/20 border border-orange-500/40 rounded-full text-[10px] font-black text-orange-400 uppercase tracking-[0.2em]">
                            <TrendingUp size={12} className="inline mr-2" /> Global Trending
                          </div>
                       )}
                    </div>
                    
                    <h1 className="text-4xl md:text-6xl font-black text-zinc-900 dark:text-white tracking-tighter leading-[1.05] drop-shadow-2xl">
                       {job.title}
                    </h1>
                    
                    <div className="flex flex-wrap items-center gap-x-8 gap-y-4 mt-8">
                       <div className="flex items-center gap-2 text-zinc-600 dark:text-zinc-300 font-bold tracking-tight">
                 ...
                 <motion.div 
                   ...
                   className="lg:w-[400px] bg-white dark:bg-zinc-900/80 backdrop-blur-3xl p-8 rounded-[40px] border border-black/5 dark:border-white/5 shadow-[0_32px_128px_-16px_rgba(0,0,0,0.1)] dark:shadow-[0_32px_128px_-16px_rgba(0,0,0,0.4)] flex flex-col gap-6"
                 >
                    ...
                    <div className="flex flex-col gap-4 mt-2">
                       {job.applyOnlineUrl && (
                         <button 
                           ...
                           className="w-full py-5 bg-blue-600 hover:bg-blue-500 text-white rounded-3xl font-black uppercase tracking-widest shadow-2xl shadow-blue-500/40 active:scale-95 transition-all text-xs"
                          >
                           Apply Now
                         </button>
                       )}
                       ...
                       {job.admitCardUrl && (
                         <button 
                           ...
                           className="w-full py-5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-3xl font-black uppercase tracking-widest shadow-2xl shadow-indigo-500/40 active:scale-95 transition-all text-xs flex items-center justify-center gap-2"
                          >
                           <ArrowLeft className="rotate-180" size={16} /> Download Admit Card
                         </button>
                       )}
                       ...
                       {job.resultUrl && (
                         <button 
                           ...
                           className="w-full py-5 bg-green-600 hover:bg-green-500 text-white rounded-3xl font-black uppercase tracking-widest shadow-2xl shadow-green-500/40 active:scale-95 transition-all text-xs flex items-center justify-center gap-2"
                          >
                           <ArrowLeft className="rotate-180" size={16} /> Check Final Result
                         </button>
                       )}
        ...
        <div className="max-w-7xl mx-auto px-8 grid grid-cols-1 lg:grid-cols-12 gap-16 py-24">
           ...
           <div className="lg:col-span-8 space-y-16">
              <section className="space-y-8">
                 ...
                 <div 
                   className="text-zinc-600 dark:text-zinc-400 text-lg leading-relaxed font-medium rich-text-content prose prose-zinc dark:prose-invert max-w-none prose-headings:text-zinc-900 dark:prose-headings:text-white prose-a:text-blue-500"
                       ...
                       >
                         <div className="flex items-center gap-6">
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-300 ${
                              activeAccordion === acc.id ? 'bg-blue-600 text-white' : 'bg-zinc-100 dark:bg-white/5 text-zinc-500 group-hover:text-blue-500'
                            }`}>
                               <acc.icon size={24} />
                            </div>
                            <span className={`text-xl font-black uppercase tracking-tighter ${activeAccordion === acc.id ? 'text-zinc-900 dark:text-white' : 'text-zinc-500 group-hover:text-zinc-700 dark:group-hover:text-zinc-300'}`}>
                              {acc.label}
                            </span>
                         </div>
                         <div className={`p-2 rounded-full border border-black/5 dark:border-white/5 transition-transform duration-500 ${activeAccordion === acc.id ? 'rotate-90 text-blue-500 border-blue-500/20' : 'text-zinc-400'}`}>
           ...
           <div className="lg:col-span-4 space-y-12">
              <div className="p-8 bg-zinc-50 dark:bg-zinc-900/40 rounded-[40px] border border-black/5 dark:border-white/5 space-y-8">
                 <h3 className="text-xl font-bold text-zinc-900 dark:text-white uppercase tracking-tighter border-b border-black/5 dark:border-white/5 pb-6">Official Sources</h3>
                 <div className="space-y-4">
                    {[
                      ...
                      <button 
                        ...
                        disabled={!link.url}
                        className="w-full flex items-center justify-between p-5 bg-white dark:bg-white/5 hover:bg-zinc-100 dark:hover:bg-white/10 rounded-2xl transition-all border border-black/5 dark:border-transparent disabled:opacity-30 group"
                      >
                         <div className="flex items-center gap-4">
                            <link.icon size={20} className="text-blue-500" />
                            <span className="text-xs font-bold text-zinc-500 group-hover:text-zinc-900 dark:group-hover:text-white uppercase tracking-widest">{link.label}</span>
                         </div>
                         <ChevronRight size={16} className="text-zinc-300 dark:text-gray-700" />
                      </button>
                    ))}
                 </div>
              </div>
        ...
        {job.relatedJobs?.length > 0 && (
          <section className="py-24 bg-zinc-50 dark:bg-zinc-900/20 border-t border-black/5 dark:border-white/5">
             <div className="max-w-7xl mx-auto px-8">
                <div className="flex items-end justify-between mb-16">
                   <div>
                      <h2 className="text-4xl font-black text-zinc-900 dark:text-white tracking-tighter uppercase">Related Positions</h2>
                      <p className="text-zinc-500 mt-2 font-bold uppercase tracking-widest text-[10px]">Opportunities matching this profile</p>
                   </div>
                   <button onClick={() => router.push('/jobs')} className="hidden md:flex items-center gap-2 text-blue-600 font-black uppercase tracking-widest text-[10px] hover:translate-x-2 transition-transform">
                      View All Archives <ArrowLeft size={16} className="rotate-180" />
                   </button>
                </div>
